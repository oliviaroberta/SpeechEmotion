...
export function ResultCard({ result }: ResultCardProps) {
  return <section className="result-card" aria-labelledby="result-title">
    ...
    {result ? <div className="result-summary" aria-live="polite"><div><span className="result-label">Detected emotion</span><strong className="emotion-result">{result.emotion}</strong></div><div className="confidence-block"><span className="result-label">Confidence</span><strong>{percent(result.confidence)}</strong></div></div> : <div className="empty-result" aria-live="polite"><Sparkles size={20} aria-hidden="true" /><p>Select a WAV recording and run an analysis to see the returned prediction.</p></div>}
