...
export function UploadPanel({ file, error, onFileSelected, onRemove }: UploadPanelProps) {
  ...
  return <section className="upload-panel" aria-labelledby="upload-title">
    <div className="section-heading"><div className="section-icon" aria-hidden="true"><UploadCloud size={18} /></div><div><p className="eyebrow">Step 1</p><h2 id="upload-title">Choose a voice recording</h2></div></div>
    <div className="drop-zone" onDragOver={(event) => event.preventDefault()} onDrop={onDrop}><FileAudio size={28} aria-hidden="true" /><p>Drag your WAV file here, or select it from your device.</p><label className="file-picker" htmlFor={inputId}>Choose WAV file</label><input id={inputId} aria-label="Choose WAV file" type="file" accept=".wav,audio/wav" onChange={inputChange} /><span>WAV files up to 4 MiB</span></div>
