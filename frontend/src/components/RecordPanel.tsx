...
export function RecordPanel({ analysing, error, onReady, onDiscard, onError, onAnalyse }: RecordPanelProps) {
  ...
  return <section className="record-panel" role="tabpanel" aria-labelledby="record-tab"><div className="section-heading"><div className="section-icon" aria-hidden="true"><Mic size={18} /></div><div><p className="eyebrow">Step 1</p><h2>Record your voice</h2></div></div>
    <div className="recording-stage" aria-live="polite">{state === 'recording' ? <><span className="recording-dot" aria-hidden="true" /><strong>Recording</strong><span>{time(duration)} / 00:15</span></> : state === 'requesting' ? <p>Waiting for microphone permission...</p> : state === 'ready' ? <p>Your recording is ready: {time(duration)}. Listen to it or check the emotion.</p> : <p>Speak for 1 to 15 seconds in a quiet place. Your recording stays on this page until you check it.</p>}</div>
    {error && <p className="form-error" role="alert">{error}</p>}
    <div className="record-controls">{state === 'idle' && <button className="analyse-button" type="button" onClick={() => void start()}><Mic size={17} aria-hidden="true" /> Start recording</button>}{state === 'recording' && <button className="stop-button" type="button" onClick={() => void finish()}><Square size={16} aria-hidden="true" /> Stop recording</button>}{state === 'ready' && <><button className="secondary-button" type="button" onClick={() => void togglePlayback()} aria-label={playing ? 'Pause recording' : 'Play recording'}><Play size={16} aria-hidden="true" /> {playing ? 'Pause' : 'Play recording'}</button><button className="secondary-button" type="button" onClick={() => void again()}><RotateCcw size={16} aria-hidden="true" /> Record again</button><button className="analyse-button" type="button" disabled={analysing || !recording} onClick={onAnalyse}>{analysing ? 'Checking...' : 'Check emotion'}</button></>}</div>
